...
function decode58(s:string):Uint8Array{const bytes=[0];for(const c of s){const v=B58.indexOf(c);if(v<0)throw Error('base58');let carry=v;for(let i=0;i<bytes.length;i++){carry+=bytes[i]*58;bytes[i]=carry&255;carry>>=8;}while(carry) {bytes.push(carry&255);carry>>=8;}}for(let i=0;i<s.length&&s[i]==='1';i++)bytes.push(0);return Uint8Array.from(bytes.reverse());}
